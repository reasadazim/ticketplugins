...
class EVO_generator extends EVO_Cal_Time{
	...
	// date and time for calendar @4.6.2
		function get_date_time_format(){
			$this->date_format = get_option('time_format');
			$this->time_format = get_option('date_format');

			// if custom date time format enabled
			if( EVO()->cal->check_yn('evo_timeF','evcal_1')){

				$date_format = EVO()->cal->get_prop('evo_timeF_v','evcal_1');
				$this->date_format = empty( $date_format ) ? 'F j(l)': $date_format;

				$time_format = EVO()->cal->get_prop('evo_timeF_tf','evcal_1');
				$this->time_format = empty( $date_format ) ? 'g:ia': $time_format;
			}
		...
		public function generate_event_data(
			...
			foreach($event_list_array as $event_):
				...
				// CLASES - attribute
					...
					// if event color is whiter @4.6.2
					$_eventClasses [] = ( eventon_is_hex_dark( $event_color )) ? 'clrD':'clrW';
