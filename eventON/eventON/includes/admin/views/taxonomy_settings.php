	...
	foreach( $fields as $key=>$value){
		...
		// get field value
			...
			if(empty($value['value'])){
				...
				if( empty($value['var']) ) continue;
				$field_id = $value['var'];

				if(!empty( $field_id ) && !empty( $termMeta[ $field_id ] )){
					if( !is_array($termMeta[ $field_id ]) && !is_object($termMeta[ $field_id ])){
						$field_value = stripslashes(str_replace('"', "'", (esc_attr( $termMeta[ $field_id ] )) ));
					}	

					if( is_array( $termMeta[ $field_id ] ))	{
						$field_value = $termMeta[ $field_id ];
					}				
				}
